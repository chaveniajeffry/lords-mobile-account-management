...
def createResearchUpgradeDefenses(request):
    if request.method == "POST":
        name = request.POST.get("name")
        level = request.POST.get("level")
        account_id = request.POST.get("pk")
        lm_account = get_object_or_404(LMUserAccount, id=account_id)
        lm_research = LMUserResearch.objects.get(account=lm_account)
        LMUserResearchUpgradeDefenses.objects.create(
            name=name,
            level=level,
            research=lm_research,
        )
...
def updateResearchUpgradeDefenses(request,pk):
    lm_user_research_upgrade_defenses = get_object_or_404(LMUserResearchUpgradeDefenses, id=pk)
    lm_user_research = get_object_or_404(LMUserResearch, id=lm_user_research_upgrade_defenses.research.id)
    lm_account = get_object_or_404(LMUserAccount, id=lm_user_research.account.id)
    lm_user_research_upgrade_defenses_form = LMUserResearchUpgradeDefensesForm(instance=lm_user_research_upgrade_defenses)
    if request.method == "POST":
        name = request.POST.get("name")
        level = request.POST.get("level")
        lm_user_research_upgrade_defenses.name = name
        lm_user_research_upgrade_defenses.level = level
        lm_user_research_upgrade_defenses.save()
        return redirect("read-lm-account", pk=lm_account.id)
    context = {
        "lm_account":lm_account,
        "lm_user_research_upgrade_defenses_form":lm_user_research_upgrade_defenses_form,
    }
    return render(request,'lm_accounts/lm_research/lm_research_upgrade_defenses_update.html',context)

def deleteResearchUpgradeDefenses(request,pk):
    lm_user_research_upgrade_defenses = get_object_or_404(LMUserResearchUpgradeDefenses, id=pk)
    lm_user_research = get_object_or_404(LMUserResearch, id=lm_user_research_upgrade_defenses.research.id)
    lm_account = get_object_or_404(LMUserAccount, id=lm_user_research.account.id)
    lm_user_research_upgrade_defenses.delete()
    return redirect("read-lm-account", pk=lm_account.id)